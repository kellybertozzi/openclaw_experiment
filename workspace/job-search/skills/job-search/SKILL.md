...
This skill is responsible for finding, researching, evaluating, ranking, and preparing job opportunities.

This skill does NOT execute job applications.

When Kelly explicitly approves one or more jobs for application:

1. Confirm which specific jobs were approved.
2. Preserve the verified job title, company, location, and direct application URL.
3. Hand the approved job(s) to the `job-apply` skill.
4. Do not independently submit an application.
5. Do not perform browser-based form filling as part of job search.

The `job-apply` skill is responsible for application execution, form filling, submission, application-state tracking, and reporting.

### Approval Boundary

Explicit application authorization includes statements such as:
...
General interest is not submission authorization.
...
do not authorize submission.

When authorization is ambiguous, ask Kelly before initiating application execution.
